...
class FishMoneyUI:
    ...
    def create_context_menu(self):
        ...
        self.menu.add_command(
            label="暂停计费",
            command=lambda: self._run_menu_action(self.toggle_pause),
            font=menu_font,
        )
        self.menu_pause_index = 0
        self.menu.add_command(
            label="详情",
            command=lambda: self._run_menu_action(self.open_details),
            font=menu_font,
        )
        self.menu.add_command(
            label="重新配置…",
            command=lambda: self._run_menu_action(self.open_settings),
            font=menu_font,
        )
        self.menu.add_command(
            label="重置今日金额",
            command=lambda: self._run_menu_action(self.reset_today),
            font=menu_font,
        )
        self.menu.add_separator()
        self.menu.add_command(
            label="退出",
            command=lambda: self._run_menu_action(self.confirm_exit),
            font=menu_font,
        )
